...
if ( ! class_exists( 'BSF_License_Manager' ) ) {
	...
	class BSF_License_Manager {
		...
		public function process_license_deactivation( $product_id ) {
			...
			if ( ! is_wp_error( $response ) || wp_remote_retrieve_response_code( $response ) === 200 ) {
				...
				if ( isset( $result['success'] ) && ( true === $result['success'] || 'true' === $result['success'] ) ) {
					...
					$_POST['bsf_license_deactivation']['success'] = isset( $result['success'] ) ? $result['success'] : false;
					$_POST['bsf_license_deactivation']['message'] = isset( $result['message'] ) ? $result['message'] : __( 'An unexpected error occurred.', 'astra-addon' );
				} else {
					$_POST['bsf_license_deactivation']['success'] = false;
					$_POST['bsf_license_deactivation']['message'] = __( 'Could not communicate with the license server. Please try again later.', 'astra-addon' );
				}
			} else {
		...
		public function bsf_process_license_activation( $post_data ) {
			...
			if ( ! is_wp_error( $response ) || wp_remote_retrieve_response_code( $response ) === 200 ) {
				...
				if ( isset( $result['success'] ) && ( true === $result['success'] || 'true' === $result['success'] ) ) {
					...
					$res['success'] = isset( $result['success'] ) ? $result['success'] : false;
					$res['message'] = isset( $result['message'] ) ? $result['message'] : __( 'An unexpected error occurred.', 'astra-addon' );
				} else {
					$res['success'] = false;
					$res['message'] = __( 'Could not communicate with the license server. Please try again later.', 'astra-addon' );
				}
			} else {
				$res['success'] = false;
		 ...
		 * @param int $product_id Product ID.
		 */
		public static function bsf_is_active_license( $product_id ) {return true;
